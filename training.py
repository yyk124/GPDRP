...
def predicting(model, device, loader):
    ...
    with torch.no_grad():
        for data in loader:
            ...
            test = pd.DataFrame(columns=['1'], data=total_preds.numpy())
...
def main(modeling, train_batch, val_batch, test_batch, lr, num_epoch, log_interval, cuda_name):
    ...
    not os.path.isfile(processed_data_file_test))):
        print('please run gin_training.py to prepare data in pytorch format!')
    else:
        ...
        for epoch in range(num_epoch):
            ...
            # drug_emb = get_embedding(model, device, all_loader)
            # drug_emb = drug_emb.cpu().numpy()
            # np.savetxt('drug_emb.csv', drug_emb, delimiter=',')
            ...
            if ret[1] < best_mse:
                ...
                with open('predictions.csv', 'w', newline='') as csvfile:
                    ...
                    for i in range(len(G_test)):
                        writer.writerow([G_test[i], P_test[i]])
...
if __name__ == "__main__":
    ...
    modeling = [GINConvNet][args.model]
